...
const settingObjForm = {
  postParams: {
    ...
    _setParams: function (answers) {
      ...
      this.slug = "301";
      this.source = "SberLanding3.8";
      this.utm_medium = "b02a2310-2ccf-4f2a-aaf3-349621468776";
...
async function sendSms(number) {
  ...
  try {
    // URL до PHP-файла
    const url = `send_sms.php?number=${correctNumber}`;
    ...
    // Обрабатываем ответ от PHP
    const result = await response.text(); // или `await response.json()` если ожидается JSON

    console.log(result); // Логируем ответ на консоль
...
function validPhoneNumber(inputArr) {
  ...
  inputArr.forEach(item => {
    ...
    if (phoneInt) {
      const errorContainer = phoneInput.closest('.form-group').querySelector('.error-phone');
...
const postData = async (data) => {
  ...
  // Используем флаг для предотвращения повторных вызовов sendSms
  let smsSent = false;
  ...
  try {
    ...
    if (result?.success === true) {
      ...
      const withoutPlus = settingObjForm.postParams.phone.replace(/^\+/, '');


      
      // Проверяем, был ли вызван sendSms ранее
      if (!smsSent) {
        smsSent = true; // Устанавливаем флаг, чтобы предотвратить повторные вызовы

        sendSms(withoutPlus).then((res) => {
          if (res) {
          ...
          // Выполняем действия после отправки SMS
          window.removeEventListener("beforeunload", trackClose);
          thenkYouPage();
        }).catch((error) => {
          console.error("Ошибка при отправке SMS:", error);

          // Все равно вызываем thenkYouPage при ошибке
          window.removeEventListener("beforeunload", trackClose);
          thenkYouPage();
        });
      }
...
document.querySelectorAll('._main-form').forEach(form => {
  form.addEventListener("submit", async (e) => {
    ...
    const inputs = form.querySelectorAll('input[name="name"], input[name="last_name"], input[name="email"], ._phone');
    ...
    const isValidForm = Array.from(inputs).every(input => input.classList.contains('valid'));

    if (isValidForm) {
      addLoader(form.querySelectorAll('.buttonSend'), form.querySelectorAll('.btnFormText'));
      settingObjForm.postParams._setParams(); // Установка параметров
      await postData(settingObjForm.postParams); // Отправка данных
      removeLoader(form.querySelectorAll('.buttonSend'), form.querySelectorAll('.btnFormText'));
