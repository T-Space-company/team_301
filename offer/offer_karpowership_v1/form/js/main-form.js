...
var adset_name = url.searchParams.get("adset_name");
var ad_name = url.searchParams.get("ad_name");
...
const settingObjForm = {
  postParams: {
    ...
    _setParams: function (answers) {
      ...
      this.landing = "karpowership";
      this.slug = "karpowership";
      this.source = "FB";
      this.utm_medium = "0a5d2299-015f-46f5-8658-61fd364dbee4";
      this.campaing_id = "karpowership";
...
window.intlTelInput(phone, {
    ...
    geoIpLookup: function (callback) {
      $.get(
        ...
        document.querySelectorAll('input[name="code"]').forEach(item => item.value = countryCode.toLocaleUpperCase());
...
form.addEventListener("submit", (e) => {
  ...
  // console.log(formErrorContent)
    ...
    if (validForm && validFormPhone) {
      addLoader(allBtnSubmit,btnFormText)
        //PHONE VALIDATION
      $.getJSON(`https://phonevalidation.abstractapi.com/v1/?api_key=4eecc1211a4b4ca2b0e56c03ac7addb5&phone=${mnc}${phoneNumberCurrent}`, function(data) {
  		})
      .then((data) => {
        if(data.valid) {
