...
var countryData = {
  ...
  kz: {
    phoneRegex: /^7[0-9]{9}$/i,
    numLimit: 10,
    phoneccValue: "7",
    countryName: "Kazakhstan",
    phonePlaceholder: "771 000 9998",
  },
  tr: {
    phoneRegex: /^5[0-9]{9}$/i,
    numLimit: 10,
    phoneccValue: "90",
    countryName: "Turkey",
  ...
  in: {
    ...
    phoneccValue: "91",
    countryName: "India",
    phonePlaceholder: "81234 56789",
  },
  md: {
    phoneRegex: /^[0-9]{8,9}$/i,
    numLimit: 9,
    phoneccValue: "373",
    countryName: "Moldova",
  ...
  az: {
    phoneRegex: /^[0-9]{9,12}$/i,
    numLimit: 12,
    phoneccValue: "994",
    countryName: "Azerbaijan",
    phonePlaceholder: "40 123 45 67",
  },
  am: {
    phoneRegex: /^[0-9]{8,10}$/i,
    numLimit: 10,
    phoneccValue: "374",
    countryName: "Armenia",
    phonePlaceholder: "77 123456",
  },
  by: {
    phoneRegex: /^[0-9]{9}$/i,
    numLimit: 9,
  ...
  uz: {
    ...
    phonePlaceholder: "90 1234567",
  },
  ee: {
    phoneRegex: /^[0-9]{7,8}$/i,
    numLimit: 8,
    phoneccValue: "372",
    countryName: "Estonia",
  ...
  ge: {
    ...
    phoneccValue: "995",
    countryName: "Georgia",
    phonePlaceholder: "555 123456",
  },
};

var countryValue = Array.from(
  new Set(
...
function numRule(countryValue) {
  ...
  if (!countryData.hasOwnProperty(countryValue)) {
    ...
    var matchingValue = Object.keys(countryData)
      .sort()
  ...
  $(".phone").each(function () {
    ...
    var flag = $('<img class="phone-country">').attr(
      "src",
      "assets/landing/img/flags/" + countryValue + ".webp"
    );
    var label = $("<label>");
    $(this).wrap(wrapper);
  ...
  if (countryList.length > 1 && countryListOne !== true) {
    ...
    for (var i = 0; i < countryList.length; i++) {
      ...
      if (caseError !== true) {
        var listItem = $("<li>")
          .addClass("country-li")
          .attr("data-code", countryCode);
        list.append(listItem);
        var listCountryFlag = $("<img>")
          .addClass("country-img")
          .attr("src", "assets/landing/img/flags/" + countryCode + ".webp");
        listItem.append(listCountryFlag);
        var listCountryName = $("<span>")
          .addClass("country-name")
          .text(countryName);
        listItem.append(listCountryName);
        var listCountryCode = $("<span>")
          .addClass("country-code")
          .text("+" + phoneccValue);
  ...
  $("form").each(function () {
    if ($(this).find("input[name=forename]").length > 0) {
      ...
      $(this)
        .find("input")
        .attr("style", function (index, value) {
          if (value === undefined) {
            value = "";
          }
          if (value.indexOf("background-color") === -1) {
            value += "background-color:" + "transparent" + " !important;";
          }
          return value;
        });
      var inpClr = $(this).find("input").css("color");
      $(this)
        .find(".phone-wrap-flag label")
        .attr("style", function (index, value) {
          if (value === undefined) {
            value = "";
          }
          return value + "color:" + "#fff" + " !important;";
        });
      $(this)
        .find(".phone-wrap-flag .country-arrow")
        .attr("style", function (index, value) {
          if (value === undefined) {
            value = "";
          }
          return value + "border-top-color:" + "#000" + " !important;";
        });
      ...
      $(this)
        ...
        .attr("style", function (index, value) {
          ...
          return (
            ...
            inpFntSz +
            " + " +
            (parseInt(paddingValue) + parseInt(arrowWidth)) +
            "px) !important;"
          );
        });
      $(this)
        ...
        .attr("style", function (index, value) {
          ...
          return value + "background-color:" + "transparent" + " !important;";
        });

      if (
        $(this).find("input[name=phonecc]").length > 0 ||
        $(this).find("input[name=country]").length > 0
      ) {
...
$.validator.addMethod(
  ...
  function (value, element) {
    return (
      this.optional(element) || /^[\p{Letter}\p{Mark}\- ]{2,25}$/u.test(value)
    );
  },
  ""
);
...
function functionSuccess(data) {
  $(".leadForm button[name='submit']").removeAttr("disabled");
  $(".leadForm button[name='submit']").text(buttonText);
  $(".leadForm button[name='submit']").removeClass("loading-btn");
...
function functionError() {
  ...
  window.location.href =
    ...
    "/?&success";
  //$("body").empty();
  //$("body").load("success.php");
}

function functionBeforeSend() {
  buttonText = $("form button[name='submit']").text();
  $(".leadForm button[name='submit']").attr("disabled", true);
  $(".leadForm button[name='submit']").text("");
  $(".leadForm button[name='submit']").addClass("loading-btn");
}

$(".leadForm button[name=submit]").click(function () {
  var form = $(this).parents("form:first");
  if (form.valid()) {
    var data = $(this).closest("form").serialize();
    $.ajax({
      url: "api.php?" + window.location.search.replace("?", ""),
      type: "POST",
      data: data,
      dataType: "html",
      error: functionError,
